...
bool failsafeIsSNLActive(void)
{
    return failsafeState.active && (failsafeConfig->failsafe_procedure == FAILSAFE_PROCEDURE_SNL);
}
        ...
        switch (failsafeState.phase) {
            ...
            case FAILSAFE_RX_LOSS_DETECTED:
                if (receivingRxData) {
                    ...
                    switch (failsafeConfig->failsafe_procedure) {
                        ...
                        case FAILSAFE_PROCEDURE_SNL:
                            // Execute STOP & LAND
                            failsafeActivate();
                            initSnlState(); // Make sure SNL knows about activation
                            break;
                    }
                }
                reprocessState = true;
            ...
            case FAILSAFE_LANDING:
                ...
                if (armed) {
                    ...
                    beeperMode = BEEPER_STOP_AND_LAND;
                }
                if (((failsafeConfig->failsafe_procedure != FAILSAFE_PROCEDURE_SNL) && failsafeShouldHaveCausedLandingByNow()) || !armed) {
                    // SNL has it's own disarm mechanism - apply this only for legacy LAND method
