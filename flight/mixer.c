...
#if defined(CONNEXFC)
    { 4, true, mixerQuadX },           // MIXER_QUADX_CONNEXFC
#else
...
#if defined(CONNEXFC)
static const servoMixer_t servoMixerConnexFCQuadX[] = {
    { SERVO_CONNEXFC_1, INPUT_RC_AUX3, 100, 0, 0, 100, 0 },
    { SERVO_CONNEXFC_2, INPUT_RC_AUX4,  100, 0, 0, 100, 0 },
};
#endif
...
#if defined(CONNEXFC)
    { COUNT_SERVO_RULES(servoMixerConnexFCQuadX), servoMixerConnexFCQuadX },    // MULTITYPE_QUADX_CONENXFC
#else
...
#if defined(CONNEXFC)
        case MIXER_QUADX:
            for (int i = SERVO_CONNEXFC_1; i <= SERVO_CONNEXFC_2; i++) {
                pwmWriteServo(servoIndex++, servo[i]);
            }
#endif
...
void mixTable(void)
{
    ...
    #define THROTTLE_CLIPPING_FACTOR    0.25f
    if (FLIGHT_MODE(ALTHOLD_DIRECT) || FLIGHT_MODE(ALTHOLD_ASSIST)) {
        // Don't allow mixer to increase overall thrust by limiting amount of correction in ALTHOLD modes
        throttleRange = MIN((throttleMax - throttleCommand), (throttleCommand - throttleMin)) * 2;

        // Allow 25% correction in any case to prevent destabilisation at very low throttle
        throttleRange = MAX(throttleRange, (throttleMax - throttleMin) * THROTTLE_CLIPPING_FACTOR);
    }
    else {
