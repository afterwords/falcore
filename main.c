...
void init(void)
{
...
#if defined(CONNEXFC)
    // Enable hardware features on the PDB
    do {
        gpio_config_t gpio_config;

        // AUX_BUFF_EN
        gpio_config.pin = Pin_2;
        gpio_config.mode = Mode_Out_OD;
        gpio_config.speed = Speed_10MHz;
        gpioInit(GPIOB, &gpio_config);
        digitalLo(GPIOB, Pin_2);

        // VIS_SYS_BUFF_EN
        gpio_config.pin = Pin_3;
        gpio_config.mode = Mode_Out_OD;
        gpio_config.speed = Speed_10MHz;
        gpioInit(GPIOC, &gpio_config);
        digitalLo(GPIOC, Pin_3);
    ...
    initBoardAlignment(&masterConfig.calibration.boardAlignment);
