...
// Rotate 2D vector by arbitrary angle; rot_angle is defined in deg*10
void rotVector2D(float Vector[2], int16_t rot_angle, float Result[2])
{
    float sinA = sin_approx(rot_angle * M_PIf / 1800.0f);
    float cosA = cos_approx(rot_angle * M_PIf / 1800.0f);
