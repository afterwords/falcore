...
#define MSG_RATE_1000MSEC				1	//in Hz
#define MSG_RATE_200MSEC				5	//in Hz
#define MSG_RATE_100MSEC				10	//in Hz
...
int16_t             mav_debug[4] = {0,0,0,0};
#endif

/* MAVLink datastream rates in Hz */
static const uint8_t mavRates[] = {
		[MAV_DATA_STREAM_POSITION] = 2, //2Hz
		[MAV_DATA_STREAM_EXTRA1] = MSG_RATE_1000MSEC, // Heartbeat, AUTOPILOT_VERSION
		[MAV_DATA_STREAM_EXTRA2] = MSG_RATE_100MSEC,  // VFR_HUD, PROSIGHT_OSD (MAV ID 150), PROSIGHT_OSD_DEBUG (MAV ID 151)
		[MAV_DATA_STREAM_EXTRA3] = MSG_RATE_200MSEC,  // Ignition key request
};
...
static falcore_ignition_t ignition_data;
...
static uint16_t mavlinkRxSerialBytesWaiting(void)
{
	return serialRxBytesWaiting(mavlinkPort);
}

static uint16_t mavlinkSerialRead(uint8_t * buf, uint16_t buf_size)
{
	uint16_t bytesToRead;
	uint16_t idx;

	bytesToRead = (uint8_t) serialRxBytesWaiting(mavlinkPort);

	for (idx = 0; (idx < bytesToRead) && (idx < buf_size); idx++)
	{
		buf[idx] = serialRead(mavlinkPort);
	}
	return idx;
}
...
void mavlinkSendHeartbeatMsg(void)
{
	uint16_t msgLength;
...
void mavlinkSendAutopilotVersionMsg(void)
{
	uint16_t 					msgLength;
	falcore_ver_t 				version;
	prosight_protocol_ver_t		protocl_ver;

	version.fields.sw_ver_minor = FC_VERSION_MINOR;
	version.fields.sw_ver_major = FC_VERSION_MAJOR;
	version.fields.sw_board = FC_VERSION_BOARD;
	version.fields.family = FC_VERSION_FAMILY;

	protocl_ver.fields.minor = PROSIGHT_OSD_PROTOCOL_VERSION_MINOR;
	protocl_ver.fields.major = PROSIGHT_OSD_PROTOCOL_VERSION_MAJOR;

	mavlink_msg_autopilot_version_pack(0, 200, &mavMsg,
			// capabilities accroding to MAV_PROTOCOL_CAPABILITY (unused)
			0,
			// Falcore autopilot version
			version.bits,
			// middleware (ProSight OSD protocol version)
			protocl_ver.bits,
			// os_sw_version (unused)
			0,
			// board_version (unused)
			0,
			// flight_custom_version (unused)
			NULL,
			// middleware_custom_version (unused)
			NULL,
			// os_custom_version (unused)
			NULL,
			//vendor_id (used as part of magic number)
			FALCORE_AMIMON_VENDOR_ID,
			//product_id (used as part of magic number)
			FALCORE_AMIMON_PRODUCT_ID,
			//uid (unused)
			0);
...
void mavlinkSendProsightOsdMsg(void)
{
	uint16_t 			msgLength;
	int16_t				altitude;
	uint8_t				flight_mode;
	...
	altitude = prosight_osd__get_altitude();
	msg_id = prosight_osd__get_msg_data();
	flight_mode = prosight_osd__get_flight_mode();
	aah_thr_pos = prosight_osd__get_aah_throttle_position();
	flags = prosight_osd__get_flags();

	mavlink_msg_prosight_osd_pack(0, 200, &mavMsg,
			altitude, msg_id.bits, flags.bits, flight_mode, aah_thr_pos);
...
#ifdef MAVLINK_DEBUG_MESSAGE
void mavlinkSendProsightOsdDebugMsg(void)
{
	uint16_t 			msgLength;

	mavlink_msg_prosight_osd_debug_pack(0, 200, &mavMsg, mav_debug[0], mav_debug[1], mav_debug[2], mav_debug[3]);
...
void mavlinkSendProsightIgnition(void)
{
    uint16_t            msgLength;

    prosight_osd__ignition_key_generate_challenge(&ignition_data);
    mavlink_msg_prosight_osd_ignition_key_pack(0, 200,  &mavMsg,ignition_data.challenge, ignition_data.token);

    msgLength = mavlink_msg_to_send_buffer(mavBuffer, &mavMsg);
    mavlinkSerialWrite(mavBuffer, msgLength);
}

void handleMAVlinkMsg(void)
{
    mavlink_prosight_osd_ignition_key_t raw_ignition_key;
	switch (mavRecvMsg.msgid)
	{
		case MAVLINK_MSG_ID_HEARTBEAT:      // MAV ID: 0
			break;

		case MAVLINK_MSG_ID_PROSIGHT_OSD_IGNITION_KEY:  // MAV ID 152
			mavlink_msg_prosight_osd_ignition_key_decode(&mavRecvMsg,&raw_ignition_key);
			ignition_data.challenge = raw_ignition_key.key;
			ignition_data.token = raw_ignition_key.token;
			prosight_osd__set_ignition_key_status (prosight_osd__ignition_key_check_challenge_response(&ignition_data));
			break;
...
void mavlinkReceiveMsg()
{
	uint16_t mavlinkBytesWaiting;
	uint8_t result;
	uint8_t c;

	mavlinkBytesWaiting = mavlinkRxSerialBytesWaiting();

	while (mavlinkBytesWaiting)
	{
	    mavlinkBytesWaiting--;
	    c = serialRead(mavlinkPort);
	    result = mavlink_parse_char(0, c, &mavRecvMsg, &mavRecvStatus);
	    if (result == MAVLINK_FRAMING_OK)
	    {
	        handleMAVlinkMsg();
	        // limit recv handle for 1 message per cycle
	        break;
	    }
	}
}
...
void processMAVLinkTelemetry(void)
{
	...
	if (mavlinkStreamTrigger(MAV_DATA_STREAM_EXTRA1)) {
		...
		mavlinkSendAutopilotVersionMsg();
	}

	if (mavlinkStreamTrigger(MAV_DATA_STREAM_EXTRA2)) {
		mavlinkSendHudMsg();
		mavlinkSendProsightOsdMsg();
#ifdef MAVLINK_DEBUG_MESSAGE
		mavlinkSendProsightOsdDebugMsg();
#endif
	}

#ifdef USE_PROSIGHT_IGNITION
	if ( (getProsightIgnitionStatus()) && (!prosight_osd__is_ignition_key_valid()) )
	{
		if (mavlinkStreamTrigger(MAV_DATA_STREAM_EXTRA3)) {
		    mavlinkSendProsightIgnition();
		}
	}

	// read from mavlink
	if ( mavlinkRxSerialBytesWaiting() )
	{
		mavlinkReceiveMsg();
	}
#endif
