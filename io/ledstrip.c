...
extern bool isCalibrating(void);
extern bool isHardwareFailing(void);
...
const ledConfig_t defaultLedStripConfig[] = {
    { CALCULATE_LED_XY(15,  7), 0, LED_DIRECTION_EAST | LED_FUNCTION_FLIGHT_MODE | LED_FUNCTION_WARNING | LED_FUNCTION_ARM_STATE },
    { CALCULATE_LED_XY(15,  0), 0, LED_DIRECTION_NORTH | LED_FUNCTION_FLIGHT_MODE | LED_FUNCTION_WARNING | LED_FUNCTION_ARM_STATE },
    { CALCULATE_LED_XY(0,   0), 0, LED_DIRECTION_NORTH | LED_FUNCTION_FLIGHT_MODE | LED_FUNCTION_WARNING | LED_FUNCTION_ARM_STATE },
    { CALCULATE_LED_XY(0,   7), 0, LED_DIRECTION_WEST | LED_FUNCTION_FLIGHT_MODE | LED_FUNCTION_WARNING | LED_FUNCTION_ARM_STATE },
    { CALCULATE_LED_XY(0,  15), 0, LED_DIRECTION_SOUTH | LED_FUNCTION_FLIGHT_MODE | LED_FUNCTION_WARNING | LED_FUNCTION_ARM_STATE },
    { CALCULATE_LED_XY(15, 15), 0, LED_DIRECTION_SOUTH | LED_FUNCTION_FLIGHT_MODE | LED_FUNCTION_WARNING | LED_FUNCTION_ARM_STATE }
};
...
void applyLedModeLayer(void)
{
    ...
    for (ledIndex = 0; ledIndex < ledCount; ledIndex++) {
        ...
        if (!(ledConfig->flags & LED_FUNCTION_FLIGHT_MODE)) {
            ...
            // Apply mode colours only when ARMED
            if (ARMING_FLAG(ARMED) || !(ledConfig->flags & LED_FUNCTION_ARM_STATE)) {
                ...
                } else if (FLIGHT_MODE(ALTHOLD_DIRECT)) {   // SH1
                    applyDirectionalModeColor(ledIndex, ledConfig, &baroModeColors);
                } else if (FLIGHT_MODE(HORIZON_MODE)) {     // Horizon
            ...
            else if (ledConfig->flags & LED_FUNCTION_ARM_STATE) {
                    setLedHsv(ledIndex, &hsv_green);
            }
        }
    }
...
typedef enum {
    ...
    WARNING_FLAG_CALIBRATING = (1 << 3),
    WARNING_FLAG_HARDWARE_FAILING = (1 << 4),
    WARNING_FLAG_RX_LOSS_DISARMED = (1 << 5),
} warningFlags_e;
...
void applyLedWarningLayer(uint8_t updateNow)
{
    ...
    if (isHardwareFailing()) {
        // A special warning case - hardware failure
        for (ledIndex = 0; ledIndex < ledCount; ledIndex++) {
            ledConfig = &ledConfigs[ledIndex];
        ...
        if (updateNow && warningFlashCounter == 0) {
            ...
            if ((feature(FEATURE_FAILSAFE) && failsafeIsActive()) || (!ARMING_FLAG(ARMED) & !failsafeIsReceivingRxData())) {
                if (feature(FEATURE_FAILSAFE) && failsafeIsActive()) {
                    warningFlags |= WARNING_FLAG_FAILSAFE;
                }
                else {
                    warningFlags |= WARNING_FLAG_RX_LOSS_DISARMED;
                }
                warningFlags &= ~WARNING_FLAG_ARMING_DISABLED;
            }
            if (isCalibrating()) {
                warningFlags |= WARNING_FLAG_CALIBRATING;
                warningFlags &= ~WARNING_FLAG_ARMING_DISABLED;
            }
        }

        if (warningFlags || warningFlashCounter > 0) {
            // Choosing ambient color
            const hsvColor_t *warningColor = &hsv_black;

            if (warningFlags & WARNING_FLAG_RX_LOSS_DISARMED) {
                warningColor = &hsv_green;
            }

            // Choosing flash color
            if ((warningFlashCounter & 1) == 0) {
                if (warningFlashCounter < 4 && (warningFlags & WARNING_FLAG_ARMING_DISABLED)) {
                    warningColor = &hsv_green;
                }
                if (warningFlashCounter >= 4 && warningFlashCounter < 6 && (warningFlags & WARNING_FLAG_CALIBRATING)) {
                    warningColor = &hsv_green;
                }
                if (warningFlashCounter >= 6 && warningFlashCounter < 12 && (warningFlags & WARNING_FLAG_LOW_BATTERY)) {
                    warningColor = &hsv_red;
                }
                if (warningFlashCounter >= 12 && warningFlashCounter < 16 && (warningFlags & (WARNING_FLAG_FAILSAFE | WARNING_FLAG_RX_LOSS_DISARMED))) {
