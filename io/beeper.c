...
static const beeperTableEntry_t beeperTable[] = {
    ...
    { BEEPER_ENTRY(BEEPER_STOP_AND_LAND,            1, beep_sos,              "STOP_AND_LAND") },
    { BEEPER_ENTRY(BEEPER_RX_LOST,                  2, beep_txLostBeep,       "RX_LOST") },
    { BEEPER_ENTRY(BEEPER_DISARM,                   3, beep_disarmBeep,       "DISARM") },
    { BEEPER_ENTRY(BEEPER_ARM,                      4, beep_armingBeep,       "ARM")  },
    ...
    { BEEPER_ENTRY(BEEPER_FLM_CHANGE,               9, beep_shortBeep,        "FLM_CHANGE") },
    { BEEPER_ENTRY(BEEPER_ACC_CALIBRATION_COMPLETE, 10, beep_2shortBeeps,     "ACC_CALIBRATION_COMPLETE") },
    { BEEPER_ENTRY(BEEPER_ACC_CALIBRATION_FAIL,     11, beep_2longerBeeps,    "ACC_CALIBRATION_FAIL") },
    { BEEPER_ENTRY(BEEPER_READY_TO_ARM,             12, beep_readyBeep,       "READY_TO_ARM") },
    ...
    { BEEPER_ENTRY(BEEPER_DISARM_DURATION_WARNING,  14, beep_disarmRepeatBeep,"DISARM_DURATION_WARNING") },
...
{
    if (mode == BEEPER_SILENCE || !getBeeperEnabledStaus()) {
        beeperSilence();
        return;
    }
...
void beeperUpdate(void)
{
    ...
    if (!beeperIsOn) {
        ...
        if (currentBeeperEntry->sequence[beeperPos] != 0) {
            ...
            if (
                beeperPos == 0
                && (currentBeeperEntry->mode == BEEPER_ARM || currentBeeperEntry->mode == BEEPER_ARMING_GPS_FIX)
            ) {
