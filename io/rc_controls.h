...
typedef enum {
    ...
    BOXBLACKBOX,
    BOXFAILSAFE,
    BOXDIRECTION_YAW,       // AMIMON: New box mode - direction
    BOXAIRMODE,             // AMIMON: New box mode - air mode (cudos to Boris B. @ rcgroups)
    BOXSNL,                 // AMIMON: New box mode - panic mode - attempt to reduce total energy and land the drone safely
    BOXCRASH,               // AMIMON: New box mode - crash mode
    BOXMODELOCK,            // AMIMON: Mode switch lock
    CHECKBOX_ITEM_COUNT
} boxId_e;

extern uint32_t rcModeActivationMask;

#define RC_MODE_MASK(modeId)        (1 << (modeId))
#define IS_RC_MODE_ACTIVE(modeId)   (RC_MODE_MASK(modeId) & rcModeActivationMask)
#define ACTIVATE_RC_MODE(modeId)    (rcModeActivationMask |= RC_MODE_MASK(modeId))

#define LOCKABLE_MODES_MASK (RC_MODE_MASK(BOXANGLE) |           \
                             RC_MODE_MASK(BOXHORIZON) |         \
                             RC_MODE_MASK(BOXAHDIRECT) |        \
                             RC_MODE_MASK(BOXAHASSIST) |        \
                             RC_MODE_MASK(BOXRATE1) |           \
                             RC_MODE_MASK(BOXRATE2) |           \
                             RC_MODE_MASK(BOXRATE3) |           \
                             RC_MODE_MASK(BOXDIRECTION_YAW) |   \
                             RC_MODE_MASK(BOXCRASH))
...
#define PWM_TO_RANGE(x)     (((x) - 900) / 25)
...
typedef struct controlRateConfig_s {
    ...
    uint16_t yaw_p_limit;                   // Additional limit on P-term on yaw
} controlRateConfig_t;

extern int16_t rcCommand[4];
extern int16_t rcStickCommand[4];
