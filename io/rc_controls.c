...
#define MIN_RC_TICK_INTERVAL_MS     25

static escAndServoConfig_t *escAndServoConfig;
static pidProfile_t *pidProfile;
static bool modeLockIsActive = false;

// true if arming is done via the sticks (as opposed to a switch)
static bool isUsingSticksToArm = true;

int16_t rcCommand[4];           // Command after HEADFREE, MAGHOLD etc, interval [1000;2000] for THROTTLE and [-500;+500] for ROLL/PITCH/YAW
int16_t rcStickCommand[4];      // Command from RX right after expo and deadband is applied; interval [1000;2000] for THROTTLE and [-500;+500] for ROLL/PITCH/YAW
    ...
    if (isUsingSticksToArm) {
        ...
        if (rcSticks == THR_LO + YAW_LO + PIT_CE + ROL_CE) {
            if (ARMING_FLAG(ARMED))
                ...
                beeper(BEEPER_DISARM_DURATION_WARNING);    // sound tone while stick held
...
bool isModeLockActive(void)
{
    return modeLockIsActive;
}

void updateActivatedModes(modeActivationCondition_t *modeActivationConditions)
{
    static uint32_t modeLockActivatedModes;
    bool modeLockNewState;
    ...
    if (!(ARMING_FLAG(ARMED) || IS_RC_MODE_ACTIVE(BOXARM))) {
        modeLockNewState = false;
    }
    else {
        modeLockNewState = modeLockIsActive || IS_RC_MODE_ACTIVE(BOXMODELOCK);   // Can't be deactivated when armed
    }

    if (modeLockNewState) {
        if (!modeLockIsActive) {
            // Activating mode lock - record lockable modes
            modeLockActivatedModes = rcModeActivationMask & LOCKABLE_MODES_MASK;
            modeLockIsActive = true;
        }
        else {
            // Lock already active - enforce modes
            rcModeActivationMask &= ~LOCKABLE_MODES_MASK;
            rcModeActivationMask |= modeLockActivatedModes & LOCKABLE_MODES_MASK;
        }
    }
    else {
        modeLockIsActive = false;
    }

    if (IS_RC_MODE_ACTIVE(BOXRATE1)) {
        changeControlRateProfile(0);
    }
    else if (IS_RC_MODE_ACTIVE(BOXRATE2)) {
        changeControlRateProfile(1);
    }
    else if (IS_RC_MODE_ACTIVE(BOXRATE3)) {
