...
bool mag3110detect(mag_t *mag)
{
    ...
    ack = i2cRead(MAG3110_MAG_I2C_ADDRESS, MAG3110_MAG_REG_WHO_AM_I, 1, &sig, MAG3110_BUS);
    if (!ack || sig != 0xC4)
        return false;
...
void mag3110Init()
{
    ...
    ack = i2cWrite(MAG3110_MAG_I2C_ADDRESS, MAG3110_MAG_REG_CTRL_REG1, 0x01, MAG3110_BUS); //  active mode 80 Hz ODR with OSR = 1
    delay(20);

    ack = i2cWrite(MAG3110_MAG_I2C_ADDRESS, MAG3110_MAG_REG_CTRL_REG2, 0xA0, MAG3110_BUS); // AUTO_MRST_EN + RAW
    delay(10);
}
...
bool mag3110Read(int16_t *magData)
{
    ...
    ack = i2cRead(MAG3110_MAG_I2C_ADDRESS, MAG3110_MAG_REG_STATUS, 1, &status, MAG3110_BUS);
    if (!ack || (status & BIT_STATUS_REG_DATA_READY) == 0) {
        return false;
    }

    ack = i2cRead(MAG3110_MAG_I2C_ADDRESS, MAG3110_MAG_REG_HXL, 6, buf, MAG3110_BUS);
