...
bool ms56xxDetect(baro_t *baro, baroSensor_e type)
{
    ...
    ack = i2cRead(MS56XX_ADDR, CMD_PROM_RD, 1, &sig, MS56XX_BUS);
    if (!ack)
        return false;
...
static void ms56xx_reset(void)
{
    i2cWrite(MS56XX_ADDR, CMD_RESET, 1, MS56XX_BUS);
...
{
    uint8_t rxbuf[2] = { 0, 0 };
    i2cRead(MS56XX_ADDR, CMD_PROM_RD + coef_num * 2, 2, rxbuf, MS56XX_BUS); // send PROM READ command
...
{
    uint8_t rxbuf[3];
    i2cRead(MS56XX_ADDR, CMD_ADC_READ, 3, rxbuf, MS56XX_BUS); // read ADC
    return (rxbuf[0] << 16) | (rxbuf[1] << 8) | rxbuf[2];
}

static void ms56xx_start_ut(void)
{
    i2cWrite(MS56XX_ADDR, CMD_ADC_CONV + CMD_ADC_D2 + ms56xx_osr, 1, MS56XX_BUS); // D2 (temperature) conversion start!
}
...
static void ms56xx_start_up(void)
{
    i2cWrite(MS56XX_ADDR, CMD_ADC_CONV + CMD_ADC_D1 + ms56xx_osr, 1, MS56XX_BUS); // D1 (pressure) conversion start!
}
